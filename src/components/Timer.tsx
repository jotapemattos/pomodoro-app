...
const Timer = ({ totalSeconds, setPomodoro, setShortBreak, setLongBreak, start, setStart, method }: TimerProps) => {
  const { pomodoroTime, shortBreakTime, longBreakTime, setPomodoroTime, setShortBreakTime, setLongBreakTime } =
  ...
  //every time the seconds update, the percentage is updated together
  useEffect(() => {
  ...
  //decrease the seconds in pomodoro time
  useInterval(() => {
    if (method === 'pomodoro') {
  ...
  //decrease the seconds in short break time
  useInterval(() => {
    if (method === 'shortbreak') {
  ...
  //decrease the seconds in long break time
  useInterval(() => {
    if (method === 'longbreak') {
  ...
  return (
    <div className="flex flex-col items-center justify-around gap-8">
      <div className="w-3/4 xl:w-full">
        {method === 'pomodoro' && (
          <CircularProgressbar strokeWidth={4} value={percentagePomodoro} text={`${convertTime(pomodoroTime)}`} />
        )}
        {method === 'shortbreak' && (
          <CircularProgressbar strokeWidth={4} value={percentageShort} text={`${convertTime(shortBreakTime)}`} />
        )}
        {method === 'longbreak' && (
          <CircularProgressbar strokeWidth={4} value={percentageLong} text={`${convertTime(longBreakTime)}`} />
        )}
      </div>
